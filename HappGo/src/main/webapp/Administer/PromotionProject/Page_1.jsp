...
<body>
	...
	<form name="myForm1">
		促銷條件一： <select size=5 onChange="renew1(this.selectedIndex);">
			<option value="1">1_消費紅利(單一條件)
			<option value="2">2_消費紅利(多條件)
			<option value="3">3_會員資料紅利
		</select> 促銷條件二： <select name="member1" size=5>
	...
	<script>
		...
		function renew1(index) {
			for (var i = 0; i < department[index].length; i++)
				document.myForm1.member1.options[i] = new Option(
						department[index][i], department[index][i]); // 設定新選項
			document.myForm1.member2.length = department[index].length; // 刪除多餘的選項
		}
	</script>
	...
	<script>
		...
		function renew2(index) {
			for (var i = 0; i < department[index].length; i++)
				document.myForm2.member2.options[i] = new Option(
						department[index][i], department[index][i]); // 設定新選項
			document.myForm2.member2.length = department[index].length; // 刪除多餘的選項
		}
	</script>
	<h3>可獲得紅利</h3>
